...
class selected_players_widget extends StatefulWidget {
  final VoidCallback updateCallback; //Callback Function
  const selected_players_widget({super.key,required this.updateCallback});
  @override
  State<selected_players_widget> createState() => _selected_players_widgetState();
}

class _selected_players_widgetState extends State<selected_players_widget> {
  @override
  Future<List<Map<String, dynamic>>> fetchApplicants() async {
    final response =
    await http.get(Uri.parse('http://192.168.0.106:3010/selected_applicant_list?'));
  ...
  void navigateToProfilePage(BuildContext context, Map<String, dynamic> data) async{
    ...
    final result= await Navigator.push(
      context,
      PageTransition(
        child: selected_player_final_profile_page(applicant: data),
