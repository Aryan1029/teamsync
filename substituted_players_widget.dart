...
import '../Screens/selected_player_final_profile_page.dart';
import '../Screens/substituted_player_final_profile_page.dart';

class substituted_players_widget extends StatefulWidget {
  final VoidCallback updateCallback; //Callback Function
  const substituted_players_widget({super.key,required this.updateCallback});
  @override
  State<substituted_players_widget> createState() => substituted_players_widgetState();
}

class substituted_players_widgetState extends State<substituted_players_widget> {
  @override
  Future<List<Map<String, dynamic>>> fetchApplicants() async {
    final response =
    await http.get(Uri.parse('http://192.168.0.106:3012/substituted_applicant_list?'));
  ...
  void navigateToProfilePage(BuildContext context, Map<String, dynamic> data) async{
    ...
    final result= await Navigator.push(
      context,
      PageTransition(
        child: substituted_player_final_profile_page(applicant: data),
