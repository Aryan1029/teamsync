...
class applicant_list_widget extends StatefulWidget {
  final VoidCallback updateCallback; //Callback Function
  const applicant_list_widget({super.key,required this.updateCallback});
  @override
  State<applicant_list_widget> createState() => _applicant_list_widgetState();
}

class _applicant_list_widgetState extends State<applicant_list_widget> {
  @override
  Future<List<Map<String, dynamic>>> fetchApplicants() async {
    final response =
    await http.get(Uri.parse('http://192.168.0.106:3000/applicant_list?'));
    // await http.get(Uri.parse('http://10.100.47.19:3000/applicant_list?'));
  ...
  void navigateToProfilePage(BuildContext context, Map<String, dynamic> data) async{
    ...
    final result= await Navigator.push(
      context,
      PageTransition(
        child: ApplicantProfilePage(applicant: data),
